...
# Title and header
st.markdown(
    "<h1 style='text-align: center; color: #800020;'>🍷 Wine Quality Predictor</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    "<h3 style='text-align: center; color: #7f8c8d;'>A Production-Grade ML Tool for Quality Assessment</h3>",
    unsafe_allow_html=True,
)
st.write("---")
...
if st.session_state.token is None:
    st.info(
        "🔒 Please register or login using the sidebar to access the predictor tool."
    )
    ...
    with tabs[0]:
        ...
        st.write(
            "Fill in the chemical values below to predict the wine quality score (0 to 10 scale)."
        )
        ...
        with col2:
            ...
            total_sulfur = st.slider(
                "Total Sulfur Dioxide (mg/L)", 5.0, 300.0, 115.0, 1.0
            )
        with col3:
            density = st.number_input(
                "Density (g/cm³)", 0.9850, 1.0100, 0.9950, 0.0001, format="%.4f"
            )
        ...
        if st.button("Predict Quality", type="primary"):
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    st.error(
                        f"Error calling predict endpoint: {res.json().get('detail', 'Unknown error')}"
                    )
            except Exception as e:
    ...
    with tabs[1]:
        ...
        st.write(
            "Upload a CSV file containing chemical stats of multiple wines. The system will append predictions and stream back the file."
        )

        st.info(
            "💡 Your CSV must contain columns similar to: `fixed acidity`, `volatile acidity`, `citric acid`, `residual sugar`, `chlorides`, `free sulfur dioxide`, `total sulfur dioxide`, `density`, `pH`, `sulphates`, `alcohol`, and `type` (or `wine_type`)."
        )
            ...
            if st.button("Generate Batch Predictions", type="primary"):
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(
                            f"Error running batch: {res.json().get('detail', 'Unknown error')}"
                        )
                except Exception as e:
    ...
    with tabs[2]:
        ...
        st.write(
            "Below are the logged single wine quality predictions stored dynamically in the audit database."
        )
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if not history_logs:
                    st.warning(
                        "No logged predictions found in the database. Go generate some predictions first!"
                    )
                else:
                    ...
                    chart_data = (
                        df_logs.groupby("wine_type")["predicted_quality"]
                        .mean()
                        .reset_index()
                    )
                    st.bar_chart(
