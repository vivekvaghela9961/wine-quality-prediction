...
def detect_drift(reference_path: str, min_records: int = 10):
    ...
    if not os.path.exists(reference_path):
        print(
            f"Reference training data not found at {reference_path}. Cannot perform drift check."
        )
        return None
    ...
    try:
        logs = (
            db.query(PredictionLog).order_by(PredictionLog.id.desc()).limit(500).all()
        )
    finally:
        db.close()
    ...
    if len(logs) < min_records:
        print(
            f"Insufficient prediction logs (found {len(logs)}, need at least {min_records}) to analyze drift."
        )
