...
def test_signup_login_flow(client, db_session):
    ...
    signup_conflict_res = client.post("/auth/signup", json={"username": username, "password": "different_password"})
    assert signup_conflict_res.status_code == 400
    
    # 3. Login invalid credentials
    login_fail_res = client.post("/auth/login", json={"username": username, "password": "wrong_password"})
    assert login_fail_res.status_code == 401
    
    # 4. Login successful
    login_success_res = client.post("/auth/login", json={"username": username, "password": password})
...
def test_prediction_logging_to_db(client, db_session):
    ...
    latest_log = db_session.query(PredictionLog).order_by(PredictionLog.id.desc()).first()
